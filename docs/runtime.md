...
The pinned Ubuntu image contains systemd `255.4-1ubuntu8.17`, which lacks an [upstream login-service fix](https://github.com/systemd/systemd/pull/36364). An exited session can leave `systemd-logind` spinning and delay provisioning. Before Lima's login setup, luhmen denies `pidfd_open` in that service while retaining its existing syscall restrictions. Sessions for which logind would open a pidfd use numeric PID and FIFO tracking instead, without pidfd-based PID-reuse protection. PAM can still pass an existing pidfd over D-Bus, so this does not disable every pidfd in logind. Docker's process tracking is unaffected. The workaround applies only to that exact systemd package version and removes its own configuration if the version changes.
...
New VMs enable Docker's [live restore](https://docs.docker.com/engine/daemon/live-restore/) so running containers can survive a Docker daemon failure. The guest service restarts the daemon automatically. Before an intentional VM stop, luhmen's static Rust guest helper temporarily disables live restore in the running daemon so Docker can stop containers without marking them manually stopped. It restores the original configuration file before waiting for Docker to exit. A failed or interrupted preparation retains recovery information for the next stop attempt. Live restore does not keep containers running through a VM shutdown or guarantee recovery after daemon configuration changes.
